...
{
	if(CurSituation[y][x] == R_PAWN) return BA0[y][x];
	if(CurSituation[y][x] == B_PAWN) return BA1[y][x];

	return 0;
}

CEveluation::CEveluation()//构造函数
{
	//初始化每种棋子的基本价值数组
...
int CEveluation::GetRelatePiece(BYTE position[][9], int j, int i)//检查棋子所有可能落子点，与CanTouch()配合筛选出所有合法落子点
{
	nPosCount=0;
	BYTE nChessID;
	BYTE flag;
...
bool CEveluation::CanTouch(BYTE position[][9], int nFromX, int nFromY, int nToX, int nToY)//判断棋子目的位置是否合法
{
	int i,j;
...
void CEveluation::AddPoint(int x, int y)//将该位置添加为保护位置
{
